    ...
    if (response.statusCode == 200) {
      ...
      if (parsedData is Map<String, dynamic> && parsedData.containsKey('data')) {
        ...
        if (responseData is Map<String, dynamic> && responseData.containsKey('data')) {
          // Paginated response
          purchaseList = responseData['data'] as List<dynamic>;
        } else if (responseData is List) {
          // Direct array response
          purchaseList = responseData;
        } else {
          throw Exception('Invalid purchase data structure: ${responseData.runtimeType}');
        }
        
        return purchaseList.map((purchase) => PurchaseTransaction.fromJson(purchase)).toList();
      } else {
